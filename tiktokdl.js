...
const CreateFileURL = require("./src/inputUrl");
const ValidateUrlTiktok = require("./src/inputUrlsValidate");
 ...
 */
const downloadImages = async (url, imageId, imageUrlPath, tanggalan, authorUniqueId) => {
  try {
  ...
  try {
    ...
    const playAddrUrlList =
      data?.__DEFAULT_SCOPE__?.["webapp.video-detail"]?.itemInfo?.itemStruct?.video
        ?.bitrateInfo?.[0]?.PlayAddr?.UrlList;

    // Check if the JSON structure is valid and contains sufficient data
    if (!playAddrUrlList || playAddrUrlList.length < 2) {
      throw new Error("Invalid JSON structure. PlayAddr.UrlList not found or insufficient data.");
    }
 ...
 */
(async () => {
  const urlTiktok = await CreateFileURL();
  const validateUrl = ValidateUrlTiktok(urlTiktok);
  if (validateUrl.length === 0) {
    console.log("Tidak ada URL TikTok yang valid.");
    return;
  }

  // Additional query parameters for TikTok URL
  const queryParams = "?is_from_webapp=1&sender_device=pc&web_id=7221493350775866882";

  // Loop through each TikTok URL
  for (const url of validateUrl) {
    let modifiedUrl = url;
      ...
      if (url.includes("/photo/")) {
        ...
        if (
          ...
          for (let i = 0; i < imageList.length; i++) {
            ...
            // Download the image
            await downloadImages(url, imageId, [imageUrl], tanggalan, authorUniqueId, newFileName);
            console.log(`✅ Image ${imageIndex} downloaded for ${authorUniqueId}`);
          }
        } else {
              ...
              if (videoDetail) {
                ...
                if (author) {
                  ...
                  if (data !== undefined) {
                    ...
                    console.log(`✅ Video downloaded for ${authorName}_${itemStruct.id}`);
                  } else {
